import React,{useState,useEffect} from "react";
import Confetti from "react-confetti";
import {socket} from "../socket/socket"
import { useLocation,useNavigate } from "react-router-dom";
const Page = () => {
const [loading,setLoading] = useState(true)
const [msg,setMsg]=useState("")
const [val,setVal]=useState([])
const [chart, setChart] = useState([]);
const [mode,setMode]=useState("profile")
const location = useLocation();
const { id, name, teams } = location.state;
const teamicons=[{team:"Yoddhas",image:"Icons/Yoddhas Icon.webp"},{team:"Titans",image:"Icons/Titans Icon.webp"},{team:"Giants",image:"Icons/Giants Icon.webp"},{team:"Thalaivas",image:"Icons/Thalaivas Icon.webp"}]
useEffect(() => {
window.scrollTo({ top: 0, behavior: 'smooth'});
}, []);
useEffect(() => {
  if (!socket.connected) {
      socket.connect();
    }
  socket.on("pokemon-updated-data",(msg)=>{
    setVal([msg.user])
  })
}, []);
const navigate = useNavigate()
const join_room=async()=>{
  if(id.length>0 && name.length>0){
   try {
    const response = await fetch("https://miniature-toma-aliudufu-dfe931ca.koyeb.app/join-pokemon-room", {
      method: "POST", 
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({name:name,id:id}),
    });

    if (!response.ok) {
      throw new Error(`HTTP error! Status: ${response.status}`);
    }
    const result = await response.json();
  if(result?.message){
    setTimeout(()=>{
  setLoading(false)
  setMsg("Room is deleted by admin")
  },3000)
  }
  else{
  setTimeout(()=>{
  setLoading(false)
  setVal([result])
  },3000)
}
  } catch (error) {
    console.error("Error:", error);
    return null;
  }
}
}
useEffect(()=>{
  join_room()
},[id,name])
  return (
    <>
    {loading && (
<div className="w-full flex flex-col items-center justify-center gap-4 my-56">
 <img src="images/Go.webp"  className="w-72 h-54 lg:w-96 lg:h-60" />
</div>)}
{
!loading && <>
<div className="w-full h-18 flex gap-x-6 bg-sky-600">
 <img src="images/5.webp" className="w-30 h-16 ml-2 mr-2 mx-1 my-1" />
 <div className="w-full flex items-center justify-center">
<img src="images/Txt.webp" className="w-40 h-16" /> </div></div>
{msg!= "" && <p className="text-center text-md lg:text-base my-36 text-black font-bold">{msg}</p>
}
{
  msg=="" && val.length > 0 && val[0].auctionStarted ==false && val[0].contestants.filter((i)=>i.players.length == 5).length == 0 &&
  <div className="flex justify-center items-center gap-6 my-48 mx-4">
  <button onClick={()=>{
    navigate("/auction", {
  state: {
    id,
    name
  }
});
  }}
className={`px-6 py-3 text-white font-semibold rounded-xl shadow-md transition-all duration-300 bg-sky-700  hover:scale-105`}>Join Auction</button>
</div>
}
{
  msg=="" && val.length > 0 && val[0].auctionStarted ==true && val[0].contestants.filter((i)=>i.players.length == 5) == 0 &&
<>
  <div className="flex flex-col justify-center items-center gap-6 mx-4 my-36">
<p className="text-center text-md lg:text-base text-black font-bold">Waiting till auction ends....</p>
  <button onClick={()=>{
    window.location.reload()
  }}
className={`px-6 py-3 my-4 text-white font-semibold rounded-xl shadow-md transition-all duration-300 bg-sky-700  hover:scale-105`}>Restart</button>
</div>
</>
}
{
  msg=="" && val.length > 0 && val[0].auctionStarted ==false && val[0].contestants.filter((i)=>i.players.length == 5).length == 4 &&
<>
  {mode=="profile" && <p className="text-center text-sm my-2 text-black font-bold p-2">Tournament ID-: {val[0].id}</p>}
  <div className="flex justify-evenly mt-4">
<button onClick={() => {setMode("profile")}}
className={`px-4 py-2 font-bold  ${mode === "profile" ? 'border-b border-b-black text-black font-bold' : 'text-black border-b border-b-transparent'}`}>Profile</button>
 <button onClick={() => setMode("play")}
className={`px-4 py-2 font-bold  ${mode === "play" ? 'border-b border-b-black text-black font-bold' : 'text-black border-b border-b-transparent'}`}>Play 
  </button> 
  <button onClick={() => setMode("matches")}
className={`px-4 py-2 font-bold  ${mode === "matches" ? 'border-b border-b-black text-black font-bold' : 'text-black border-b border-b-transparent'}`}>Schedule
  </button> 
    <button onClick={() => setMode("stats")}
className={`px-4 py-2 font-bold  ${mode === "stats" ? 'border-b border-b-black text-black font-bold' : 'text-black border-b border-b-transparent'}`}>Teams
  </button> 
  </div>
{mode== "profile" && <>
<div className="w-full flex justify-center items-center">
<div className="w-72 h-52 bg-sky-700 my-4 flex flex-row justify-center items-center rounded-md mt-12 mb-6 lg:w-84 lg:h-90">
{ val[0].contestants.filter((it)=> it.name == name).map((i,ind)=>{
  return(<>
<div key={ind} className="w-1/2 flex flex-col justify-center items-center">
<img src={teamicons.filter((it)=> it.team == i.team)[0].image} className="w-36 h-36" />
<img src={`Logos/${teamicons.filter((it)=> it.team == i.team)[0].team}.webp`} className="w-12 h-12" />
</div>
  </>)
})
}
<div className="flex flex-col justify-center items-center">
<p className="text-center text-sm my-2 text-white font-bold">{name}</p>
  <div className="flex flex-row mt-6 justify-center gap-3 ">
<button onClick={()=>window.location.reload()} className="bg-gray-900 text-white text-base w-24 h-9 font-bold rounded-md shadow-md">Reload</button>
</div>
</div>
</div>
</div>
<div className="max-w-2xl mx-auto p-4 bg-sky-500 text-black">
<p className="text-center text-base font-bold mb-4">Points Table</p>
<table className="w-full border-collapse">
 <thead>
<tr className="border-b border-gray-600 text-black">
<th className="p-2 text-left">#</th>
<th className="p-2 text-left">Team</th>
<th className="p-2">M</th>
<th className="p-2">W</th>
<th className="p-2">L</th>
<th className="p-2">PTS</th>
<th className="p-2">NRR</th>
</tr>
</thead>
<tbody>
{val[0].contestants.slice().sort((a,b)=>{
      if (b.win !== a.win) return b.win - a.win;  
    return b.nrr - a.nrr;   
}).map((team, index) => (
<tr key={team.id} className="border-b border-gray-700 text-center font-bold">
<td className="p-2">{index + 1}</td>
<td className="p-2 flex items-center font-bold">
<img src={`Logos/${team.team}.webp`} alt={team.team} className="w-8 h-8 mr-2" />
</td>
<td className="p-2">{team.matches}</td>
<td className="p-2">{team.win}</td>
<td className="p-2">{team.lose}</td>
<td className="p-2">{2*team.win}</td>
<td className="p-2">{team.nrr=="0" ? "0.00" : team.nrr.toFixed(2)}</td>
</tr>))}
</tbody>
</table>
</div>
</>}
{mode== "play" && (val[0].winner == "" || val[0].runnerup=="" || val[0].thirdplace=="") && <> 
{
  val[0].knockouts.filter((i)=> (i.firstteam.name == name || i.secondteam.name== name) && i.winner == "").length > 0 && <>
  <div className="flex flex-col ml-2 mr-2 gap-4 my-5 justify-center items-center">
 {val[0].knockouts.filter((i)=> (i.firstteam.name == name || i.secondteam.name== name) && i.winner == "").map((i,ind)=>{
  if(ind==0)
    return(<>
    <div className="w-72 h-54 p-2 bg-sky-700 flex flex-col rounded-md flex-wrap lg:w-96 lg:h-90 md:w-96 md:h-84">
<p className="text-center text-sm text-white font-bold">{i.type}</p>
 <div className="w-full mt-2 flex flex-row">
 <div className="w-2/5 ml-2 gap-1 flex flex-col items-center justify-center">
 <img src={teamicons.filter((it)=> it.team == i.firstteam.team)[0].image} className="w-auto h-auto" />
  <img src={`Logos/${teamicons.filter((it)=> it.team == i.firstteam.team)[0].team}.webp`} className="w-12 h-12"/>
    <h1 className="text-sm text-white font-bold">{i.firstteam.name}</h1>
 </div>
  <div className="w-1/5 ml-2 mr-2 flex flex-col items-center justify-center">
<h1 className="text-sm text-white font-bold">V/S</h1>
 </div>
 <div className="w-2/5 gap-1 mr-2 flex flex-col items-center justify-center">
 <img src={teamicons.filter((it)=>it.team==i.secondteam.team)[0].image} className="w-auto h-auto" />
  <img src={`Logos/${teamicons.filter((it)=>it.team==i.secondteam.team)[0].team}.webp`} className="w-12 h-12"/>
<h1 className="text-sm font-bold text-white">{i.secondteam.name}</h1>
 </div>
    </div>
<div className="flex flex-row mt-2 justify-center gap-3 ">
<button onClick={()=>{
navigate("/play", {
  state: {
    matchID:id,
    matchtype:i.type,
    name,
    firstname:i.firstteam.name,
    secondname:i.secondteam.name,
    firstteam:i.firstteam.team,
    secondteam:i.secondteam.team,
    firstplayers:val[0].contestants.filter((x)=> x.name == i.firstteam.name)[0].players,
    secondplayers:val[0].contestants.filter((x)=> x.name == i.secondteam.name)[0].players
  }
});
}} className="bg-gray-900 text-white text-base px-6 py-2 font-bold rounded-md shadow-md"> Play</button>
 </div>
    </div>
    </>)
  })}
  </div>
  </>
}
{
  val[0].matches.filter((i)=> (i.firstteam.name == name || i.secondteam.name== name) && i.winner == "").length > 0 && <>
  <div className="flex flex-col ml-2 mr-2 gap-4 my-5 justify-center items-center">
 {val[0].matches.filter((i)=> (i.firstteam.name == name || i.secondteam.name== name) && i.winner == "").map((i,ind)=>{
  if(ind==0)
    return(<>
    <div className="w-72 h-54 p-2 bg-sky-700 flex flex-col rounded-md flex-wrap lg:w-96 lg:h-90 md:w-96 md:h-84">
<p className="text-center text-sm text-white font-bold">{i.type}</p>
 <div className="w-full mt-2 flex flex-row">
 <div className="w-2/5 ml-2 gap-1 flex flex-col items-center justify-center">
 <img src={teamicons.filter((it)=> it.team == i.firstteam.team)[0].image} className="w-auto h-auto" />
  <img src={`Logos/${teamicons.filter((it)=> it.team == i.firstteam.team)[0].team}.webp`} className="w-12 h-12"/>
    <h1 className="text-sm text-white font-bold">{i.firstteam.name}</h1>
 </div>
  <div className="w-1/5 ml-2 mr-2 flex flex-col items-center justify-center">
<h1 className="text-sm text-white font-bold">V/S</h1>
 </div>
 <div className="w-2/5 gap-1 mr-2 flex flex-col items-center justify-center">
 <img src={teamicons.filter((it)=>it.team==i.secondteam.team)[0].image} className="w-auto h-auto" />
  <img src={`Logos/${teamicons.filter((it)=>it.team==i.secondteam.team)[0].team}.webp`} className="w-12 h-12"/>
<h1 className="text-sm font-bold text-white">{i.secondteam.name}</h1>
 </div>
    </div>
<div className="flex flex-row mt-2 justify-center gap-3 ">
<button onClick={()=>{
navigate("/play", {
  state: {
    matchID:id,
    matchtype:i.type,
    name,
    firstname:i.firstteam.name,
    secondname:i.secondteam.name,
    firstteam:i.firstteam.team,
    secondteam:i.secondteam.team,
    firstplayers:val[0].contestants.filter((x)=> x.name == i.firstteam.name)[0].players,
    secondplayers:val[0].contestants.filter((x)=> x.name == i.secondteam.name)[0].players
  }
});
}} className="bg-gray-900 text-white text-base px-6 py-2 font-bold rounded-md shadow-md"> Play</button>
 </div>
    </div>
    </>)
  })}
  </div>
  </>
}
{
  val[0].knockouts.filter((i)=>i.winner == "").length > 0 && val[0].knockouts.filter((i)=> (i.firstteam.name == name || i.secondteam.name == name) && i.winner=="").length == 0 && <>
   <div className="flex flex-col ml-2 mr-2 gap-4 my-5 justify-center items-center">
  {
    val[0].contestants.filter((i)=>i.name == name).map((i)=>{ return(<>
 <div className="w-72 h-54 p-2 bg-sky-700 flex flex-col items-center justify-center rounded-md flex-wrap lg:w-96 lg:h-90 md:w-96 md:h-84">
 <img src={teamicons.filter((it)=> it.team == i.team)[0].image} className="w-40 h-40" />
  <img src={`Logos/${teamicons.filter((it)=> it.team == i.team)[0].team}.webp`} className="w-12 h-12"/>
    <h1 className="text-sm p-4 text-white font-bold">Waiting for Next match.....</h1>
 </div>
    </>)})
  }
  </div>
  </>
}
{
  val[0].knockouts.filter((i)=>i.winner=="").length == 0 &&
  <>
{
  val[0].contestants.filter((i)=> i.team == val[0].matches[0].firstteam.team || i.team == val[0].matches[0].secondteam.team).filter((i)=>
    i.name == name).length > 0 && val[0].winner!=="" && val[0].runnerup!=="" && val[0].thirdplace=="" && <>
   <div className="flex flex-col ml-2 mr-2 gap-4 my-5 justify-center items-center">
  {
    val[0].contestants.filter((i)=>i.name == name).map((i)=>{ return(<>
 <div className="w-72 h-54 p-2 bg-sky-700 flex flex-col items-center justify-center rounded-md flex-wrap lg:w-96 lg:h-90 md:w-96 md:h-84">
 <img src={teamicons.filter((it)=> it.team == i.team)[0].image} className="w-40 h-40" />
  <img src={`Logos/${teamicons.filter((it)=> it.team == i.team)[0].team}.webp`} className="w-12 h-12"/>
    <h1 className="text-sm p-4 text-white font-bold">Waiting for Results.....</h1>
 </div>
    </>)})
  }
  </div>
</>
}
{
  val[0].contestants.filter((i)=> i.team == val[0].matches[1].firstteam.team || i.team == val[0].matches[1].secondteam.team).filter((i)=>
    i.name == name).length > 0 && val[0].thirdplace!=="" && val[0].winner=="" && val[0].runnerup== "" && <>
   <div className="flex flex-col ml-2 mr-2 gap-4 my-5 justify-center items-center">
  {
    val[0].contestants.filter((i)=>i.name == name).map((i)=>{ return(<>
 <div className="w-72 h-54 p-2 bg-sky-700 flex flex-col items-center justify-center rounded-md flex-wrap lg:w-96 lg:h-90 md:w-96 md:h-84">
 <img src={teamicons.filter((it)=> it.team == i.team)[0].image} className="w-40 h-40" />
  <img src={`Logos/${teamicons.filter((it)=> it.team == i.team)[0].team}.webp`} className="w-12 h-12"/>
    <h1 className="text-sm p-4 text-white font-bold">Waiting for Results.....</h1>
 </div>
    </>)})
  }
  </div>
</>
}
  </>
}
</>
}
{ mode== "play" && (val[0].winner != "" && val[0].runnerup!="" && val[0].thirdplace!="") && <> 
  <Confetti
      width={window.innerWidth}
      height={window.innerHeight}
      numberOfPieces={450}
      recycle={false}
    />
<div className="w-full flex justify-center items-center my-8 gap-x-4 gap-y-4 flex-row flex-wrap">
<div className="w-full flex justify-center items-center">
<img src="Icons/trophy.webp"  className="w-16 h-16" />
</div>
<div className="flex justify-center items-center flex-col gap-2">
<p className="text-black text-xs font-bold">Winner</p>
<img src={teamicons.filter((i)=> i.team == val[0].winner)[0].image} className="w-24 h-24" />
<img src={`Logos/${teamicons.filter((i)=> i.team == val[0].winner)[0].team}.webp`} className="w-8 h-8" />
<p className="text-center text-xs text-black font-bold">{val[0].contestants.filter((i)=> i.team == val[0].winner )[0].name}</p>
</div>
<div className="flex justify-center items-center flex-col gap-2">
<p className="text-black text-xs font-bold">Second</p>
<img src={teamicons.filter((i)=> i.team == val[0].runnerup)[0].image} className="w-24 h-24" />
<img src={`Logos/${teamicons.filter((i)=> i.team == val[0].runnerup)[0].team}.webp`} className="w-8 h-8" />
<p className="text-center text-xs text-black font-bold">{val[0].contestants.filter((i)=> i.team == val[0].runnerup)[0].name}</p>
</div>
<div className="flex justify-center items-center flex-col gap-2">
<p className="text-black text-xs font-bold">Third</p>
<img src={teamicons.filter((i)=> i.team == val[0].thirdplace)[0].image} className="w-24 h-24" />
<img src={`Logos/${teamicons.filter((i)=> i.team == val[0].thirdplace)[0].team}.webp`} className="w-8 h-8" />
<p className="text-center text-xs text-black font-bold">{val[0].contestants.filter((i)=> i.team == val[0].thirdplace)[0].name}</p>
</div>
{ val[0].contestants.filter((i)=> i.team != val[0].winner && i.team != val[0].runnerup && i.team != val[0].thirdplace).map((it)=>{
  return(<>
  <div className="flex justify-center items-center flex-col gap-2">
<p className="text-black text-xs font-bold">Loser</p>
<img src={teamicons.filter((i)=> i.team == it.team)[0].image} className="w-24 h-24" />
<img src={`Logos/${teamicons.filter((i)=> i.team == it.team)[0].team}.webp`} className="w-8 h-8" />
<p className="text-center text-xs text-black font-bold">{val[0].contestants.filter((i)=> i.team == it.team)[0].name}</p>
</div>
  </>)
})}
</div>
</>
}
{ mode== "matches" && <> 
{ val[0].knockouts.filter((i)=> i.winner == "").length > 0 && <>
<div className="lg:w-full lg:flex lg:justify-center lg:items-center lg:flex-row lg:gap-x-24 lg:my-4 lg:flex-wrap">
{
  val[0].knockouts.map((it,ind)=>{
  return(<>
<div className="flex gap-y-6 my-10 flex-col lg:items-center lg:justify-center">
<div className="w-full flex justify-center items-center lg:w-80">
<p className="text-center text-sm text-black font-bold">Match {ind+1}</p>
</div>
<div className="w-full flex lg:w-80">
<div className="w-1/2 flex justify-center items-center flex-col gap-2">
<img src={teamicons.filter((i)=> i.team == it.firstteam.team)[0].image} className="w-32 h-32" />
<img src={`Logos/${teamicons.filter((i)=> i.team == it.firstteam.team)[0].team}.webp`} className="w-10 h-10" />
<p className="text-center text-sm text-black font-bold">{it.firstteam.name}</p>
{ it.winner!= "" && <p className="text-center text-sm text-black font-bold">{(it.winner==it.firstteam.team)?"Winner":"Loser"}</p>}
</div>
<div className="flex justify-center items-center">
<p className="text-center text-sm text-black font-bold">V/S</p>
</div>
<div className="w-1/2 flex justify-center items-center flex-col gap-2">
<img src={teamicons.filter((i)=> i.team == it.secondteam.team)[0].image} className="w-32 h-32" />
<img src={`Logos/${teamicons.filter((i)=> i.team == it.secondteam.team)[0].team}.webp`} className="w-10 h-10" />
<p className="text-center text-sm text-black font-bold">{it.secondteam.name}</p>
{ it.winner!= "" && <p className="text-center text-sm text-black font-bold">{(it.winner==it.secondteam.team)?"Winner":"Loser"}</p>}
</div>
</div>
</div>
  </>)
  })
}
</div>
</>
}
{ val[0].knockouts.filter((i)=> i.winner == "").length == 0 && <>
<div className="lg:w-full lg:flex lg:justify-center lg:items-center lg:flex-row lg:gap-x-24 lg:my-4 lg:flex-wrap">
{
  val[0].matches.map((it,ind)=>{
  return(<>
<div className="flex gap-y-6 my-10 flex-col lg:items-center lg:justify-center">
<div className="w-full flex justify-center items-center lg:w-80">
<p className="text-center text-sm text-black font-bold">{it.type}</p>
</div>
<div className="w-full flex lg:w-80">
<div className="w-1/2 flex justify-center items-center flex-col gap-2">
<img src={teamicons.filter((i)=> i.team == it.firstteam.team)[0].image} className="w-32 h-32" />
<img src={`Logos/${teamicons.filter((i)=> i.team == it.firstteam.team)[0].team}.webp`} className="w-10 h-10" />
<p className="text-center text-sm text-black font-bold">{it.firstteam.name}</p>
{ it.winner!= "" && <p className="text-center text-sm text-black font-bold">{(it.winner==it.firstteam.team)?"Winner":"Loser"}</p>}
</div>
<div className="flex justify-center items-center">
<p className="text-center text-sm text-black font-bold">V/S</p>
</div>
<div className="w-1/2 flex justify-center items-center flex-col gap-2">
<img src={teamicons.filter((i)=> i.team == it.secondteam.team)[0].image} className="w-32 h-32" />
<img src={`Logos/${teamicons.filter((i)=> i.team == it.secondteam.team)[0].team}.webp`} className="w-10 h-10" />
<p className="text-center text-sm text-black font-bold">{it.secondteam.name}</p>
{ it.winner!= "" && <p className="text-center text-sm text-black font-bold">{(it.winner==it.secondteam.team)?"Winner":"Loser"}</p>}
</div>
</div>
</div>
  </>)
  })
}
</div>
</>
}
</>
}
{
  mode=="stats" && <>
{
  val[0].winner != "" && val[0].runnerup != "" && val[0].thirdplace != "" &&
  <>
  <div className="flex flex-col justify-center items-center my-4 gap-4">
<p className="text-center font-bold text-black">Top Performers</p>
<div className="flex flex-row flex-wrap gap-4 justify-center items-center">
{
  val[0].contestants
  .flatMap(i => i.players.map(player => ({
    ...player,
    team: i.team
  })))
  .slice()
  .sort((a, b) => (b.runs + b.wickets) - (a.runs + a.wickets))
  .map((i, ind) => {
    if (ind < 6)
      return (
        <div key={ind} className="flex flex-col justify-center items-center bg-sky-700 text-white font-bold rounded-md px-3 py-4 gap-2">
          <img src={i.image} className="w-20 h-20" />
          <p>{i.name}</p>
          <img src={teamicons.find(it => it.team == i.team)?.image} className="w-12 h-12" />
        </div>
      );
  })
}
</div>
  </div>
  </>
}
<div className="flex items-center gap-3 justify-center">
<img src={`Logos/${val?.[0]?.contestants?.find((it) => it.name === name)?.team}.webp`}
alt="team logo" className="w-24 h-24 my-4"/>
</div>
<div className="w-full my-2 flex justify-center items-center  gap-3 flex-wrap flex-row">
  {
    val?.[0]?.contestants
      ?.find((c) => c.name === name)
      ?.players?.map((player, index) => (
        <div
          key={index}
          className="flex flex-col p-3 w-24"
        >
          <img
            src={player.image}
            alt={player.name}
            className="w-20 h-20"
          />
          <p className="mt-2 text-sm font-semibold text-center">
            {player.name}
          </p>
          
        </div>
      ))
  }
</div>
  </>
}
</>
}
</>
}
    </>
  );
};

export default Page;
